...
for i in range(len(measure_points_uniform)):
    measure_points_uniform[i] = boundary_nodes_params[ np.argmin(np.absolute(boundary_nodes_params- measure_points_uniform[i])) ]
...
for i in range(100):
    ...
    experiment_interpolator = interp1d(boundary_data_param, boundary_values+delta, kind='cubic')
