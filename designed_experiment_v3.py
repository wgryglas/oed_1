...
for it in range(num_iterations):
    ...
    test_new_positions = boundary_nodes_params#np.linspace(0.,1., num_test_positions)
...
#    plt.semilogy(testpoints[-1],Aoptimality[optim], 'o')
    testpoints = np.sort(testpoints)
    
    testpoints_all_combinations.append(np.copy(testpoints))
    
    print "MOVEMENT: ", np.sqrt(np.sum((testpoints-testpoints_pre)**2))
    #tmp = testpoints[-1]
    #testpoints[-1] = testpoints[work_on_point]
    #testpoints[work_on_point] = tmp            
